...
Correct five stale enumerations in `crates/grove-loop/src/task_name.rs`'s
...
Four of the five were found by `kind-slug-handle-k135` while drafting chapter 3
and are adjudicated on `docs/walkthroughs/grove-loop/03-kind-slug-handle.md`
today; the fifth is chapter 4's and `the-name-k136` is expected to adjudicate it
on `04-the-name.md` before this leaf runs. **Read both pages first** — each names
the defect and the enumeration behind it, so this leaf does not have to
re-derive them.
...
| 3 | 989 | `peel_key`'s *the two callers disagree about what an over-wide key means* | three callers, as above. Line 980's *shared by `split_shape` and `Handle::parse`* is incomplete in the same way, and line 1,004 already states the correct relation twenty-five lines later |
...
Rows 1, 2, 4 and 5 are chapter 3's bytes; row 3 is chapter 4's.
...
- **Keep the line count of every reproduced block identical.** The book's ledger
  ...
  file and silently breaks pages two finished sessions already proved. Reword
...
- All five comments state what the file holds, and no reproduced block's line
  count has changed.
