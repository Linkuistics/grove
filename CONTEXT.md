...
_Avoid_: reading the four declarations as something the filesystem shows. The
tree carries the *shape* — which passes ran, where each stopped. A cap edited
after pass 1 is visible nowhere at all, and a series that hit its cap leaves a
**byte-identical** tree to one that exited on its condition, so the escalating
session writes what happened into the series brief or the fact is lost.
_Avoid_: treating *declared pass* and *correction run* as a partition of Grove
repetition. It partitions this construct's. A leaf can also stay **live** across
several sessions behind a human gate, creating no entry at all; that is neither,
and a series does not represent it.
...
implemented flat and parameterized named forms, with structurally checked
inactive profiles, are documented in
