...
Each verb takes optional `--harness <name>`; unset, both resolve the harness this grove is stamped to, else auto-detection from the repo's harness directories (`.claude/`, `.codex/`, `.pi/`). **`grove do` is the only verb that writes the stamp** — `.grove-stamps/<name>`, keyed by the working tree's basename — whenever `--harness` is passed explicitly, and also when needed for disambiguation in multi-harness repos; a single-harness repo relying on auto-detection stays stamp-free. `grove retire --harness` picks a harness for its one by-hand session and leaves the binding alone: a lasting binding is written by the action that asks for one, and the grove is bound by the verb that drives it (`docs/adr/model-per-task-kind.md`). The grove's name is the working tree's own directory basename; the task tree itself is `.grove/` inside it.
