...
`capture-issues-for-later-groves` workstream in grove's own repo,
which is the worked example for every pattern below. Paths point at
artifacts in that workstream so the chain from concern → research →
grilling → ADR → CLI work is traceable end-to-end.
...
- The planning leaf sits in an architectural neighbourhood with
  well-known prior art (issue trackers, sync protocols, schema
  registries, build systems, etc.).
...
`050-research-in-repo-issue-trackers.md` (now retired) is the worked
...
090). Its findings de-risked the 060 sync-semantics grilling
specifically — the post-mortem survey's "Synthesis for 060" section
pre-judged most of the design tree the 060 grilling would otherwise
have had to walk.

## How to write a research-leaf brief

The single most leveraged move is to **name the downstream questions
the research must answer**, leaf by leaf. The researcher doesn't have
to guess what's load-bearing.

The 050 leaf's brief put it this way:
...
> leaf(es) its lesson informs. The final **Synthesis** section
> answers the leaf-specific questions in one place.
...
*non-obvious paradigms* (a previous prior-art survey had already
...
primary source found" notes (e.g. Google's internal use or non-use
of git-appraise, CRDT-merge surprises in Radicle COBs) — the absence
is itself a confidence signal, and recording it stops future readers
from re-doing the same fruitless search.
...
The grilling skill (bundled at `.<harness>/skills/grove/grilling.md`
after `grove install`) states it briefly: interview one question at a
time, propose a recommended answer for each, walk down the design
tree until shared understanding is reached. The moves below make
that interview productive rather than ceremonial.
...
"directory of files" (heavily pre-judged by the research). Its
...
step exists in the conversation record because the user asked for
the LLM's view before committing.
...
directions: the research doc gains a section pointing forward at the
...
See `docs/research/in-repo-issue-tracker-postmortems.md`'s "Findings
adopted" section, `docs/adr/0004-inbox-as-directory-of-observation-files.md`'s
rationale paragraph, and `docs/adr/0005-grove-meta-sync-semantics.md`'s
rationale paragraph for the worked end-to-end.

## Anti-patterns

- **The wizard.** A capture verb that opens an interactive prompt
  sequence ("title? description? type? component?") breaks mid-flow
  concentration. The prior-art evidence on this is emphatic — see
  ditz's failure mode in the postmortem survey. Capture must be one
  non-interactive gesture (flags or stdin).
...
- **The runaway tree.** Decomposition is meant to be lazy (grove
  ...
  stop and ask whether the parent question was the wrong unit. The
  020 subtree in this very workstream rolled its renumber four
  times — that pattern is exactly the signal that motivated the
  `grove leaf insert` CLI verb candidate (see leaf 080 in
  `.grove/`).
...
> before you commit and ask for pushback when it agrees too easily;
> record each settled decision inline in the task file as it lands.
> At the end, ADRs cite the research by primary source; the research
> doc gets a "Findings adopted" pointer back. That's the loop.
...
- The bundled `.<harness>/skills/grove/grilling.md` (after
  `grove install`) — the LLM-side grilling procedure.
- The bundled `.<harness>/skills/grove/driving.md` (after
  `grove install`) — the LLM-side companion to this doc.
