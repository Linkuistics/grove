...
readonly fixture="$here/fixture/bin/fzf-tmux"
readonly skill="$repo_root/plugins/linkuistics/skills/writing-code-walkthroughs/SKILL.md"
readonly expected_fixture_sha=ea14943d739fc81952d5071a642591df25ed8dac166040a6d6036b3be197cecd
...
initialize() {
  local root=$1 auth_source=$2 actual_fixture_sha actual_skill_sha
...
run_attempt() {
  ...
  mkdir -p "$home" "$run_dir/bin"
  cp -R "$template"/. "$home"/
  cp "$fixture" "$run_dir/bin/fzf-tmux"
  ...
  jq -rs '[.[] | select(.type == "item.completed" and .item.type == "agent_message")] | last | .item.text // ""' "$attempt_dir/raw.jsonl" >"$attempt_dir/final.md"
  jq -c 'select(.type == "item.started" or .type == "item.completed") | select(.item.type == "command_execution" or .item.type == "mcp_tool_call" or .item.type == "web_search")' "$attempt_dir/raw.jsonl" >"$attempt_dir/tool-events.jsonl"
  printf 'attempt=%s status=%s wall=%s final_bytes=%s tool_events=%s run_unchanged=%s\n' "$attempt_dir" "$status" "$((end_epoch - start_epoch))" "$(wc -c <"$attempt_dir/final.md" | tr -d ' ')" "$(wc -l <"$attempt_dir/tool-events.jsonl" | tr -d ' ')" "$(cmp -s "$attempt_dir/run-pre.manifest.tsv" "$attempt_dir/run-post.manifest.tsv" && printf yes || printf no)"
}

case ${1-} in
  init) initialize "$2" "$3" ;;
  run) run_attempt "$2" "$3" "$4" "$5" ;;
  *) printf 'usage: %s init ROOT AUTH_SOURCE | run ROOT ARM REPETITION ATTEMPT\n' "$0" >&2; exit 2 ;;
esac
