...
Task files are drawn from a closed set of **seventeen** kinds, **parameterised rather than flat**: five producers — **requirements** (**the grilling session** — using `grilling.md`, the procedure bundled from Matt's `grilling` and `domain-modeling` skills — to interrogate *what* should be built one question at a time, sharpening vocabulary as it goes), **design** (given requirements, establish *how*: a spec, an ADR set, or both), **planning** (cut the design into vertical slices and **grow the tree**, replacing a leaf with a node of child briefs and ordered leaves), **prototype** (a cheap throwaway artifact built to react to, not to ship), and **impl** (produces code, docs, or tests) — each with its own **review-** step (a fresh-context adversarial read producing findings, not a fix) and **integrate-review-** step (triage those findings and apply the real ones), plus **research** (a citation-disciplined survey, producing `docs/research/<slug>.md`) and **combine-research** (union two surveys and flag every disagreement). A task too big for one focused session *is* a planning task — its job is to decompose, not to do. Only planning carries methodological force — it is the loop's sole branch and the only kind that grows the tree; every other kind produces an artifact, differing in discipline and in which harness and model the self-driving loop launches it on ([`docs/adr/model-per-task-kind.md`](adr/model-per-task-kind.md)), not in what the loop does with it. The membership and each kind's discipline are [`specs/task-kind-taxonomy.md`](specs/task-kind-taxonomy.md); the set is **closed**, so an eighteenth kind is a change to grove's code and docs rather than a label a leaf may coin ([`adr/task-kind-taxonomy.md`](adr/task-kind-taxonomy.md)).

Two composition patterns run over that set, and grove **enforces neither** — a grammar is a relation *between* leaves, and grove expresses none. The **review chain** is `X` → `review-X` → `integrate-review-X`: sequential, adversarial, and each step a *different* kind, so a per-kind harness policy expresses it ("reviews go to codex"). The **vendor pair** is `research` → `research` → `combine-research`: two independent surveys unioned, where the two producers are the *same* kind differing only by vendor — which no kind→harness function can express, and is why a leaf may name its own harness. Both are the shape a session is told to reach for by default, and both are a **node directory** — `<stem>-chain/` or `<stem>-pair/` — holding steps **named off the shared stem with a terminal suffix**: `<stem>` / `<stem>-review` / `<stem>-integrate`, and `<stem>-a` / `<stem>-b` / `<stem>-combine`. The directory is what makes the group structural in every tree viewer there is — `yazi`, Finder, `ls -R` — collapsible and countable without any of them being taught the convention, which the kind itself cannot manage (it lives in the file, not the name). Each shape is **one call** — `grove-llm leaf-add-chain <parent> <stem> --kind <producer>` and `grove-llm leaf-add-pair <parent> <stem> --harness-a <name> --harness-b <other>` — which exists to perform the one derivation the kind set bought and a session would otherwise transcribe by hand: a chain's two step kinds from its producer, a pair's two vendor declarations. Hand-transcription admits a *wrong-but-well-formed* result — `--kind review-impl` beside a `design` producer is a valid invocation the write-gate cannot catch — and that is a discipline misroute, not a typo. The node is **brief-less by rule**, which is how the Retire cascade tells a chain from a decomposition without any reader parsing the suffix — a file's presence, never a name pattern — and it is why a chain node's close has nothing to do where a decomposition node's has a `Done when` to check and a brief to promote. Neither close asks the human anything: a node is never marked, so the answer would have changed nothing on disk ([`adr/confirmation-boundary.md`](adr/confirmation-boundary.md)). Nothing else changes: nothing parses the suffix, `pick` is the same stateless walk and leaves a chain node as freely as any other, and `leaf-add` still makes exactly one leaf. The verbs write the convention and enforce nothing, so skipping a chain, or cutting a partial one, stays a normal choice.
...
Each launch prints one line naming both what it resolved and what it resolved it *for* — `grove: launching claude (model: opus) — session-leaf-binding-k28 (design)`. The leaf is named by its stable `<slug>-k<key>` handle (task-tree-scheme), and the tail is omitted when there is no leaf to name (the bootstrap and finish-cycle launches). It names the leaf the driver **routed on**, which is a forecast rather than a reservation: the session picks for itself and the tree wins (model-per-task-kind), so this line and a session working a different leaf is precisely the disagreement it exists to make visible.
...
- `continue` — pick the next live leaf depth-first, bootstrap by reading, execute, commit, retire the leaf and close any node it finishes, signal completion. This is the per-task prompt the self-driving loop relaunches.
...
### Steering a grilling session

A `requirements` session is a grilling — the LLM asks one question at a time, walks down a design tree, and sharpens vocabulary as it goes. (`planning` no longer interrogates; it cuts the settled design into slices. The advice below applies to both, since both absorb surfacing concerns and both may reshape the tree.) The user's job is not to anticipate the agenda but to **redirect it as concerns surface**. Most sessions of any depth end up touching subjects neither party started with: a name that lies about its scope, a sync semantics that was silently assumed, a class of failure modes that prior tools have already mapped. These concerns rarely arrive in the order the LLM is grilling them, and they should not wait — the cost of capturing a foundational concern mid-session is one renumber; the cost of capturing it later is a migration.
...
The mechanical bookkeeping — renumbering files, updating headers, hunting cross-references, growing the parent brief — belongs to the LLM, not the user. Judgement calls (what the new concern is, what to name it, where in the order it should sit) belong to the user. The healthy session is one where the user makes a small number of substantive judgements and the LLM converts each into the right tree shape; the unhealthy one is where the user is mentally tracking the numbering. If you notice yourself doing the latter, that is a signal to pause.
