...
| `implement` / `prototype` skills | Covered by grove's work-task + `verify` skill, and "research retires into ADRs". |
| `wayfinder` tracker/DAG/claim-by-assignment | Violates grove's spine (constraints 1 & 6): grove's git-tracked tree in one worktree **is** its wayfinder. Confirmed a parallel reinvention, not a competitor. |
