...
<!-- Generated by scripts/review-yield.py at change id wvwnwnqx. The script
     was deleted with the rest of the formal-methods apparatus
     (delete-formal-models-k29); the change id is kept because it names which
     tree the counts below describe. See docs/loop-record.md for why a change
     id rather than a commit id. -->
...
`scripts/review-yield.py` separated the two and made the judgement checkable:

- **Derived.** Every ATX heading, top-level ordered-list item and table row
  in all eighteen bodies was extracted from markdown block structure at a
  pinned revision — complete by construction, with no lexical pattern
  consulted. That is **248 enumerated items**.
- **Judged.** Each was assigned to one classified *enumeration block* — the
  unit the session itself chose. That assignment table lived in the script.
- **Asserted, in both directions.** Every enumerated item had to land in
  exactly one block, and every block had to resolve to at least one item.
  The script exited non-zero if either side had a leftover, so this document
  could not be regenerated against a tree it no longer described.

That was the shape `models/run.sh` used to assert obligation coverage, and
it was here for the same reason: a clean result from a broken instrument is
indistinguishable from a clean result from a correct one.

**Both instruments are now gone, and the judged half went with them.** The
script and `models/run.sh` were deleted with the campaign's apparatus
(`delete-formal-models-k29`), and the assignment table lived inside the
script rather than in this document. So the derived half above is still
checkable in principle — the block structure of eighteen bodies at a named
revision is a fact anyone can re-extract — while the judged half is not: the
mapping from item to block is no longer written down anywhere. Read the
counts below as a measurement that was asserted in both directions when it
was taken, not as one that can be re-asserted now.
