...
`Usage: grove [COMMAND]` — bare lifecycle plus `view [WORKTREE]` and `config show [--kind KIND] [--json]` or `config examples`.
...
| G8 | `grove config examples` | Worked invocation and help; fixed personal-directory filenames; no workspace, active policy, lease or epoch dependency; full preflight, untouched matching regular files, conflict refusal, exclusive creation, partial-failure reports and retry; active/unrelated bytes preserved, no force/destination option or ignore edit; stdout/stderr and exit codes 0/1/2. |
...
|---|---|---|
| `usage-configuration-examples` | Installing inactive configuration examples |
| `usage-inspecting-configuration` | Inspecting active configuration before launch |
