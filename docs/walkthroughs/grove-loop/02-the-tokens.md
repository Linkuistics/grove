...
This chapter owns 451 of the file's 1,714 lines in three blocks: the module
...
<!-- fragment «classification-verdict-tests» owner="four-verdicts" source="crates/grove-loop/src/task_name.rs" lines="1178-1199" parent="source-task-name" -->
...
<!-- fragment «name-tests-the-charter» owner="four-verdicts" source="crates/grove-loop/src/task_name.rs" lines="1178-1185" parent="classification-verdict-tests" -->
````rust
...
**What it establishes.** `BRIEF.md` reaches `Entry` as the `Brief` variant, and
the domain's own advertisement of its distinguished child names that same
variant. Those are two directions of one fact, and the test asserts both.

**What it would still pass under.** The first assertion alone would pass while
`distinguished()` returned `None` or some other name — and a domain that parses
`BRIEF.md` correctly but advertises nothing is one where the store never asks for
a charter, so every node's brief is either read as an ordinary entry or not
looked for at all. The second assertion is what closes that, and it checks the
advertised value rather than a parse. Both assertions together would still pass
while the *store* never consulted `distinguished()` — that direction is the
library's obligation, not grove's, and it is discharged by the conformance kit
chapter 4 runs rather than here.

<!-- fragment «name-tests-foreign» owner="four-verdicts" source="crates/grove-loop/src/task_name.rs" lines="1186-1199" parent="classification-verdict-tests" -->
````rust
    #[test]
...
<!-- fragment «name-tests-kind-not-a-token» owner="four-verdicts" source="crates/grove-loop/src/task_name.rs" lines="1313-1358" parent="shape-refusal-tests" -->
````rust
    // ---- refusals inside the shape -----------------------------------------
...
<!-- fragment «name-tests-missing-separator» owner="four-verdicts" source="crates/grove-loop/src/task_name.rs" lines="1359-1395" parent="shape-refusal-tests" -->
````rust
...
<!-- fragment «name-tests-one-reading» owner="four-verdicts" source="crates/grove-loop/src/task_name.rs" lines="1396-1457" parent="shape-refusal-tests" -->
````rust
...
<!-- fragment «name-tests-node-wearing-outcome» owner="four-verdicts" source="crates/grove-loop/src/task_name.rs" lines="1458-1481" parent="shape-refusal-tests" -->
````rust
...
<!-- fragment «name-tests-bad-slug» owner="four-verdicts" source="crates/grove-loop/src/task_name.rs" lines="1482-1497" parent="shape-refusal-tests" -->
````rust
    #[test]
...
<!-- fragment «name-tests-species-mismatch» owner="four-verdicts" source="crates/grove-loop/src/task_name.rs" lines="1498-1521" parent="shape-refusal-tests" -->
````rust
