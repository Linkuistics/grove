...
The models omit these material boundaries. They do not model filesystem bytes or
...
coarser than the model atom equality. String-specific failures such as non-UTF-8
names and multi-component rendering are also outside the models. The crate tests
and the implementation's explicit species comparison cover those seams at the
Rust boundary.
...
The source ledger contains 17 roots and 8,720 owned source lines. Every
...
In the final run, the book check reports zero deferred ranges. The crate test
command runs the library, binary, integration, CLI-contract, and documentation
tests, including the guard proving that algebra modules cannot reach the
