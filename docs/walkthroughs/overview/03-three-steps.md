...
The CLI first parses the command and obtains the current directory. For
`view`, it calls the viewer with the supplied path or that directory and
returns. All remaining statements belong to the bare lifecycle. This placement
makes observation independent of jj metadata, a driver lease and launch policy.
...
<!-- fragment «surface-resolve-lease-run» owner="one-call" source="crates/grove/src/cli.rs" lines="37-73" parent="source-command-surface" -->
<!-- insert «run-seam-doc» -->
...
`main.rs` declares only the CLI module and calls its `run`. Its module comment
...
itself. The returned `anyhow::Result` makes an error print at the process
boundary with an unsuccessful exit.
...
<!-- fragment «run-seam-doc» owner="one-call" source="crates/grove/src/cli.rs" lines="37-45" parent="surface-resolve-lease-run" -->
````rust
...
|---|---|
| `Cli::parse` | Shell arguments become an optional view command; help/version exit here |
...
<!-- fragment «run-three-steps» owner="one-call" source="crates/grove/src/cli.rs" lines="59-67" parent="surface-resolve-lease-run" -->
````rust
...
<!-- fragment «run-signal-doc» owner="one-call" source="crates/grove/src/cli.rs" lines="46-54" parent="surface-resolve-lease-run" -->
````rust
...
re-raising. The viewer has already returned before this match can run.

<!-- fragment «run-call-and-endings» owner="one-call" source="crates/grove/src/cli.rs" lines="68-73" parent="surface-resolve-lease-run" -->
````rust
...
modes before that error reaches main. These all use the same returned error
channel, while the messages identify which precondition or I/O operation failed.

<!-- fragment «run-errors-doc» owner="one-call" source="crates/grove/src/cli.rs" lines="55-58" parent="surface-resolve-lease-run" -->
````rust
/// # Errors
///
