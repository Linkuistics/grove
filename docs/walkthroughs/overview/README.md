...
start, resume and finish, which is exactly the surface this book reads the
source of.

`crates/grove` is three files and 204 lines: a manifest, a thirteen-line entry
point, and one module that parses an empty command line, resolves the working
...
convention enforced only by a test. All three are provable inside these 204
lines.
...
check proves complete reconstruction of all three files and 204 lines.
