...
start, resume and finish, alongside the [read-only browser](../../USAGE.md#usage-viewing-tree).

`crates/grove` is three files and 224 lines: a manifest, a thirteen-line entry
point, and one module that dispatches an observation request or resolves the working
tree, takes the driver lease and calls the loop. That is the whole human entry
...
package it is, the grammar it accepts, the two paths it dispatches, the tests that
...
convention enforced only by a test. All three are provable inside these 224
lines.

**The book's boundary is the call.** It explains one module, `grove::cli`, and
names the rest. Everything behind `grove_tui::run` and `grove_loop::run` — the loop, the task tree,
...
check proves complete reconstruction of all three files and 224 lines.
