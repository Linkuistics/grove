...
other two plugins already are ([`../README.md`](../README.md)). It is one
**spine** skill today; one `grove-<kind>` skill per session kind lands beside it,
over the same spine.

| | |
|---|---|
| skill | `grove` — the shared spine |
...
|---|---|---|
| the seven single-kind references, and `grilling.md` | inline in `grove-<kind>` | `plugin-kind-skills-k17` |
...
| everything else | already here | this leaf |
...
name*) — so the spine is `grove:grove` there.
...
**not yet**. The spine declares `harnesses: [claude-code]`, so the script skips
it and says why. That is not a workaround for a collision; it is the declaration
being true. The binary owns `~/.codex/skills/grove` and `~/.pi/agent/skills/grove`
until `delete-provisioning-k19`, and `install.sh` refuses to install over a real
directory at a path it does not own. The declaration flips to `[any]` when the
binary stops writing those paths.
...
longer exist in the binary. Dependency-free bash over the shipped skill set:
...
- A row whose owner file is not shipped yet is **pending**, not asserted. The
  count is in every run's report and reaches zero at `plugin-kind-skills-k17`.
