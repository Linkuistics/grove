...
| **I hold two families over one subject — what reads them?** | build them independently, then **replay** each column's unique findings in the other, once | replay is the instrument, not a comparison step, and it is available only here: the two findings neither column could reach came out of one deliberate replay, and maintaining the columns in step would have cost more and found less. Budget its tax — a negative result from a bounded tool is ambiguous until a run is spent disambiguating it, and a replay pays that per finding carried across `[048, synthesis]` |
...
| **budgeting a bounded model check** | treat the **property count** as cheap; expect depth and state width to cost, and quote the depth beside the result | 58 extra invariants were nearly free — 3 at depth 3 took 373 s against 61 at depth 4 at 377 s — but that pair moves depth too, and the same model took 445 s when three added constants widened its state at no extra depth. And a depth-4 result on an eleven-step path verifies the beginning of the transaction and nothing after it `[045]` |
