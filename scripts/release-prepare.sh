...
main() {
  ...
  jj log --no-graph --reversed -r "$tag..main" \
    -T 'if(!empty, "- " ++ description.first_line() ++ "\n")' >"$notes_dir/entries"
  [[ -s "$notes_dir/entries" ]] || die "no nonempty changes to describe since $tag"
  awk -v notes="$notes_dir/entries" '
    { print }
    /^## Unreleased$/ {
      print ""
  ...
  jj new main
  echo 'release-prepare: generated and committed notes from changes since the previous tag'
}
